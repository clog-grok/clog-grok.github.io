(function () {
  ...
  css.textContent = [
    ...
    ".row input[type=\"text\"],.with-unit input[type=\"text\"],.result-line input[type=\"text\"],.plan-row input[type=\"text\"],.price-row input[type=\"text\"],.split .half input[type=\"text\"],.check input[type=\"text\"]{text-align:right;-webkit-appearance:none;appearance:none;background:var(--card-bg);color:var(--text-main);border:1px solid var(--border);border-radius:6px;}",
    ".row input[type=\"text\"]{width:104px;padding:6px 8px;font-size:0.95rem;}",
    ".with-unit input[type=\"text\"]{width:88px;}",
    ".split .half input[type=\"text\"],.split .half .with-unit input[type=\"text\"]{width:3.4em;padding:6px 4px;}",
    ".price-row input[type=\"text\"]{width:96px;font-weight:700;border-color:var(--accent);}",
    ".cap-edit{width:108px;}",
    ".need-edit{width:64px;}"
  ].join("");
  ...
  s.onload = function () {
    ...
    (function loop() { place(); tidyMoney(); requestAnimationFrame(loop); })();
    hookInputs();
    setMemo(false);
  };
  ...
  function hookInputs() {
    ...
    bridge.textContent = [
      "window.__setUnitsManual=function(v){try{unitsManual=!!v;}catch(e){}}",
      "window.__setSyncing=function(v){try{syncing=!!v;}catch(e){}}"
    ].join(";");
    ...
    Array.prototype.forEach.call(document.querySelectorAll("input"), toText);

    function selectAll(el) {
      setTimeout(function () {
        try {
          var n = String(el.value || "").length;
          if (el.setSelectionRange) el.setSelectionRange(0, n);
          else el.select();
        } catch (err) {}
      }, 30);
    }
    ...
    var coeffEl = orig("coeff");
    var effEl = orig("eff");
    var unitEl = orig("unitM2");

    function syncFromCoeff(writeSelf) {
      var c = liveNum(coeffEl.value);
      if (!isFinite(c) || c < 1) return false;
      if (window.__setSyncing) window.__setSyncing(true);
      if (writeSelf) coeffEl.value = dec2(c);
      var e = 1 / c;
      if (e > 1) e = 1;
      if (e < 0.01) e = 0.01;
      effEl.value = dec2(e);
      if (window.__setSyncing) window.__setSyncing(false);
      return true;
    }
    function syncFromEff(writeSelf) {
      var e = liveNum(effEl.value);
      if (!isFinite(e) || e <= 0) return false;
      if (e > 1) e = 1;
      if (window.__setSyncing) window.__setSyncing(true);
      if (writeSelf) effEl.value = dec2(e);
      var c = 1 / e;
      if (c < 1) c = 1;
      coeffEl.value = dec2(c);
      if (window.__setSyncing) window.__setSyncing(false);
      return true;
    }

    if (coeffEl) {
      coeffEl.addEventListener("input", function (e) {
        e.stopImmediatePropagation();
        if (window.__setUnitsManual) window.__setUnitsManual(false);
        syncFromCoeff(false);
        if (typeof rememberType === "function") rememberType();
        runCalc();
      }, true);
      coeffEl.addEventListener("blur", function () {
        syncFromCoeff(true);
        if (typeof rememberType === "function") rememberType();
        runCalc();
      });
    ...
    if (effEl) {
      effEl.addEventListener("input", function (e) {
        e.stopImmediatePropagation();
        if (window.__setUnitsManual) window.__setUnitsManual(false);
        syncFromEff(false);
        if (typeof rememberType === "function") rememberType();
    ...
    if (unitEl) {
      unitEl.addEventListener("input", function (e) {
        e.stopImmediatePropagation();
        if (window.__setUnitsManual) window.__setUnitsManual(true);
        if (typeof rememberType === "function") rememberType();
        runCalc();
      }, true);
    }
  ...
  function tidyMoney() {
    function clean(el) {
      ...
      var n = t.replace(/(\d+)\.00(?=\s*万)/g, "$1");
      if (n !== t) el.textContent = n;
    }
    clean(orig("formulaText"));
    clean(orig("steps"));
  }
