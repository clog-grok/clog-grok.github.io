...
function calc() {
  ...
  const pM2 = priceM2();
  if (m2) {
    if (dispUnit === "m2") landConv.textContent = fmt(m2 * TO_TSUBO, 1) + "坪 × 3.305785 ＝ " + fmt(m2, 1) + "㎡";
    else landConv.textContent = fmt(m2, 1) + "㎡ × 0.3025 ＝ " + fmt(m2 * TO_TSUBO, 1) + "坪";
  ...
  fillDispArea(farIn, far);
  document.getElementById("archPair").textContent = pairArea(arch);
  document.getElementById("farPair").textContent = pairArea(far);
  ...
  stepsEl.textContent = [
    fmt(m2, 1) + "㎡×建蔽" + kPct + "%＝" + fmt(arch, 1) + "㎡ ／ 容積" + yPct + "%＝" + fmt(far, 1) + "㎡",
    "専有" + fmt(usedExclusive, 1) + "㎡×" + fmt(c, 2) + "＝延べ" + fmt(usedGfa, 1) + "㎡（" + units + "戸／上限" + capUnits + "戸）",
    costLine
  ].join("\n");
  ...
  document.getElementById("stepsPills").innerHTML = pills.map(function (p) {
...
document.getElementById("extraMan").addEventListener("blur", function () {
  const man = parseCommaNum(document.getElementById("extraMan").value);
  if (isFinite(man)) document.getElementById("extraMan").value = fmtComma(man);
});
...
farIn.addEventListener("input", function () {
  if (syncing) return;
  const far = parseDispArea(farIn);
  const m2 = landM2();
  if (far > 0 && m2 > 0) {
    yoseki.value = String(Math.round((far / m2) * 1000) / 10);
  } else if (far > 0) {
    const y = (parseFloat(yoseki.value) || 0) / 100;
    if (y > 0) setLandM2(far / y);
  }
  unitsManual = false;
  calc();
});
...
function setMemoMode(on) {
  ...
  const head = document.getElementById("stepsHead");
...
document.getElementById("copySteps").addEventListener("click", function () {
  copyText(formulaDump(), document.getElementById("copySteps"), "写");
});
...
document.getElementById("copyMemo").addEventListener("click", function () {
  copyText(document.getElementById("memoBox").value || formulaDump(), document.getElementById("copyMemo"), "全体をコピー");
});
...
function loadState() {
  try {
    ...
    else if (typeof data.fsIndex === "number" && data.fsIndex >= 0 && data.fsIndex < STEPS_PX.length) stepsFsIndex = data.fsIndex;
